 ...
 * given week and had no future booking on file at the time (per that
 * week's Cancellations report) — same "genuinely not rebooked" definition
 * as getNotRebookedClients above (excludes reschedule-note cancellations
 * and the same bulk/corporate/stale exclusions as the clinic-wide stats).
 ...
 * Denominator is that week's distinct cancelling clients — same convention
 ...
 *
 * Deliberately does NOT factor in not_rebooked_resolved (the manual
 * "Dealt with" dismiss on the Unretained follow-up list) — that flag means
 * a staff member acknowledged/actioned the client, not that they actually
 * rebooked. An earlier version of this trend excluded resolved rows on the
 * theory that resolving implies rebooking, which in practice just made
 * every week's rate quietly decay to 0% a few weeks later as admin worked
 * through their to-do list, regardless of whether the client ever came
 * back — a real client outcome (next_booking) is the only signal that
 * should move this number, so each week's rate is now a fixed historical
 * fact once its report is uploaded, same as every other Cancellations-
 * report-derived KPI.
 */
export async function getDropOutRateHistory(providerName: string, weeks: string[]): Promise<DropOutRatePoint[]> {
  ...
  const { data } = await supabase
    .from("cancellation_events")
    .select("week_ending, client, note, next_booking, case_name, appointment_date, modified_at")
    .eq("provider", providerName)
    .eq("status", "Cancelled")
  ...
  return weeks.map((week_ending) => {
    ...
    const droppedOut = new Set(weekRows.filter((r) => r.next_booking === null).map((r) => r.client));
