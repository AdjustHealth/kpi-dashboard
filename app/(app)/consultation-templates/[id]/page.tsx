...
export default async function ConsultationPage({
  ...
  const rows =
    await sql`select id, form_data from assessments where id = ${id} and assess_type = 'initial_consult'`;
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Client-Centred Consult"
        showWeekSelector={false}
      />
      <ConsultWorkspace
        consultId={rows[0].id as string}
        initialNote={mergeConsultNote(formData.note)}
        initialReport={formData.report}
      />
    </>
  );
}
